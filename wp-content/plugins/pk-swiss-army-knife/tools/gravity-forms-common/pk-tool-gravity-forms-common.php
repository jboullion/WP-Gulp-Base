	...
	if(!function_exists('pk_tool_pk_form_submit_button')){
		function pk_tool_pk_form_submit_button($button, $form) {
			if($form['button']['type'] == 'text' || !$form['button']['imageUrl']) {
				return '<button type="submit" id="gform_submit_button_'.$form['id'].'" class="btn btn-primary '.apply_filters('pk_form_submit_button_classes', '').'">'.apply_filters('pk_form_submit_button', '<span><span>'.$form['button']['text'].'</span></span>', $form['button']['text']).'</button>';
			}
